...
automated_secrete_key = secrets.token_hex(254)
...
def create_access_token(data: dict):
    to_encode = data.copy()
    logging.debug(f"auto generated secrete key {automated_secrete_key} ")

    expire = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode.update({"exp": expire})
    encoded_jwt = jwt.encode(to_encode, automated_secrete_key, algorithm=ALGORITHM)
    return encoded_jwt
...
def create_refresh_token(data: dict):
    ...
    return jwt.encode(data_copy, automated_secrete_key, algorithm=ALGORITHM)


def verify_token(token: str, credentials_exception=None):
    try:
        logging.debug(f"token = {token}")
        payload = jwt.decode(token, automated_secrete_key, algorithms=[ALGORITHM])
...
def verify_refresh_token(token: str):
    try:
        decode_token = jwt.decode(token, automated_secrete_key, algorithms=[ALGORITHM])
